...
# Dictionary to map SDG names to IDs
SDG_NAME_TO_ID = {
    "No Poverty": 1,
    "Zero Hunger": 2,
    ...
    while True:
        ...
        try:
            ...
            for work in data.get('results', []):
                ...
                sdg_ids = [sdg.get('display_name', '') for sdg in sdgs]
                source_id = work.get('primary_location', {}).get('source', {}).get('id')
                # Fetch the publisher only once for the source ID
...
def calculate_sdgi_score(df):
    ...
    # Top SDG Dominance
    sdg_counts = sdg_column.str.split("; ").explode().value_counts()
    ...
    # Extract Top 3 SDGs as numeric IDs
    top_sdg_ids = [SDG_NAME_TO_ID.get(sdg, "Unknown") for sdg in top_sdgs.index]
    ...
    return {
        ...
        "Top SDG 1": top_sdg_ids[0] if len(top_sdg_ids) > 0 else "Unknown",
        "Top SDG 2": top_sdg_ids[1] if len(top_sdg_ids) > 1 else "Unknown",
        "Top SDG 3": top_sdg_ids[2] if len(top_sdg_ids) > 2 else "Unknown"
    }
...
if __name__ == "__main__":
    # Define input and output file paths and time range
    journals_csv_path = 'journals.csv'  # Path to your input CSV with journal names and ISSNs
    start_year = 2020  # Start of the publication year range
    end_year = 2024 # End of the publication year range
    output_csv = f'journal_grades_{start_year}_{end_year}.csv'  # Output file name

    # Process all journals in the input CSV and save results
